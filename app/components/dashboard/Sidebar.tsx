              ...
              {item.submenu ? (
                <div>
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-4 py-3 rounded-lg text-white hover:bg-white/10 transition-colors ${openMenus.includes(item.name) ? 'bg-white/10' : ''
                      }`}
                  >
                    ...
                    <ul className="ml-8 mt-1 space-y-1">
                      {item.submenu.map((subItem) => (
                        <li key={subItem.name}>
                          <Link
                            href={subItem.href}
                            className={`flex items-center gap-3 px-4 py-2 text-sm text-white/80 hover:text-white hover:bg-white/5 rounded-lg transition-colors ${pathname === subItem.href ? 'bg-white/10 text-white' : ''
                              }`}
                          >
                ...
                <Link
                  href={item.href || '#'}
                  className={`flex items-center gap-3 px-4 py-3 rounded-lg text-white hover:bg-white/10 transition-colors ${isActive(item.href) ? 'bg-white/10' : ''
                    }`}
                >
                  {item.icon}
                  <span className="text-sm font-medium">{item.name}</span>
